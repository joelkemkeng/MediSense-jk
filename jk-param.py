...
def read_serial_data():
    ...
    try:
        ...
        while True:
            ...
            if ser_taille.in_waiting > 0:
                data2 = ser_taille.readline().decode('utf-8').strip()  # Lit une ligne et la décode
                try:
                    taille = float(data2)  # Convertit la donnée en float
                    print(f'Votre taille est de {taille} m')  # Affiche la taille
                except ValueError:
                    print(f'Erreur de conversion des donnees pour taille : {data2}')  # Affiche une erreur si la conversion échoue
